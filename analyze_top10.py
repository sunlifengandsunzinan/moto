#!/usr/bin/env python3
"""辽宁摩旅路线 Top10 分析脚本
1. 读取 D:\摩旅数据采集\辽宁摩旅路线_全量分析.json
2. 硬逻辑初筛（相关性、里程、互动量）
3. 逐条调用 ollama qwen2.5:7b 生成专业评语
4. 排序取 Top10 输出
"""
...
def main():
    print("=" * 60)
    print("辽宁摩旅路线 Top10 分析")
    print("=" * 60)

    # 加载数据
    print("[1/4] 加载数据...")
    data = load_data()
    print(f"  共 {len(data)} 条笔记")

    # 硬逻辑初筛
    print("[2/4] 硬逻辑初筛...")
    candidates = hard_filter(data)
    print(f"  初筛合格: {len(candidates)} 条")
    ...
    for i, n in enumerate(candidates):
        ...
        safe_title = title[:30].encode('utf-8', errors='replace').decode('utf-8', errors='replace')
